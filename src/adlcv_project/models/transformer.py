...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # First split the embed_dim to num_heads x head_dim  # Secondly merge the batch_size with the num_heads
        keys = rearrange(keys, "B seq_len (nh hd) -> (B nh) seq_len hd", nh = self.num_heads, hd=self.head_dim)
        
        # HINT repeat the same process for queries and values
        queries = rearrange(queries, "B seq_len (nh hd) -> (B nh) seq_len hd", nh = self.num_heads, hd=self.head_dim)

        values = rearrange(values, "B seq_len (nh hd) -> (B nh) seq_len hd", nh = self.num_heads, hd=self.head_dim)
        ...
        attention = F.softmax(attention_logits, dim=-1) # softmax on last dimension
